...
;;;
;;;
;;;   Purpose: Example of a concrete OWLAPI reasoner bridge (for RacerPro). 
...
(defun reasoner-tbox-sync (tbox)
  #+:racer-server
  (racer::ensure-knowledge-base-state ':tbox-prepared 
				      (find-tbox tbox)))

(defun reasoner-sync (kb)
  (racer::ensure-knowledge-base-state ':abox-prepared 
				      (find-abox kb)))
...
(defun reasoner-clear-tbox-and-abox (tbox abox)
  (let ((namespaces
         (racer::tbox-namespaces (find-tbox tbox))))
    ...
    (racer::in-tbox-internal tbox
                             t
                             nil
                             racer::*default-tbox-concept-size*
                             racer::*default-tbox-role-size*)
    
    (racer::in-abox-internal abox tbox t)

    (setf (racer::tbox-namespaces (racer::find-tbox tbox))
          namespaces)))
...
(defun reasoner-dispose (reasoner)
  (setf racer::*cached-prefixes* nil)
...
(defun reasoner-ensure-role (role tbox) 
  (racer::ensure-role role (find-tbox tbox)))
...
(defun reasoner-register-subproperty (sub-property super-property)
  (let ((tbox (find-tbox (owlapi-tbox *cur-reasoner*))))
    (cond ((consp super-property) ; only inv possible
           (let ((inv-role (gentemp (format nil "~A" super-property))))
             (reasoner-ensure-role (second super-property) tbox)
             (reasoner-ensure-role sub-property tbox)
             (push inv-role (racer::tbox-internal-roles tbox))
             (inverse-of-role inv-role (second super-property) tbox)
             (role-has-parent sub-property inv-role tbox)))

          ((and (consp sub-property)
                (eq (first sub-property) 'inv)
                (not (cddr sub-property)))
           (let ((inv-role (gentemp (format nil "~A" sub-property))))
             (reasoner-ensure-role (second sub-property) tbox)
             (reasoner-ensure-role super-property tbox)
             (push inv-role (racer::tbox-internal-roles tbox))
             (inverse-of-role inv-role (second sub-property) tbox)
             (role-has-parent inv-role super-property tbox)))

          ((consp sub-property) ; composition 
           (declare-role-axiom sub-property super-property tbox))

          (t (role-has-parent sub-property super-property tbox)))))
...
(defun reasoner-define-datatype (datatype-name data-range tbox)
  ...
  #+:ignore
  (racer::define-datatype-1 datatype-name data-range tbox))
...
(defun reasoner-process-annotation (entity annotation-role annotation-value abox)
  (if (and (consp annotation-value)
      ...
      (add-annotation-concept-assertion
       abox
       entity
       `(d-filler ,annotation-role ,annotation-value))
...
(defun reasoner-forget-concept-assertion (abox ind concept)
  (forget-concept-assertion abox ind concept))
...
(defun reasoner-forget-datatype-role-filler (abox subject literal role)
  (forget-datatype-role-filler abox subject literal role))

(defun reasoner-forget-negative-datatype-role-filler (abox subject literal role)
  (forget-negative-datatype-role-filler abox subject literal role))

(defun reasoner-forget-role-assertion (abox subject object role)
  (forget-role-assertion abox subject object role))
...
(defun reasoner-atomic-role-parents (role tbox)
  ...
  (reasoner-sync tbox)
  (let ((parents  
         (racer::atomic-role-parents2 role tbox)))
...
(defun reasoner-atomic-role-children (role tbox)
  ...
  (reasoner-sync tbox)
  (let ((children
         (racer::atomic-role-children2 role tbox)))
...
(defun reasoner-atomic-role-ancestors (role tbox)
  ;;; for OWLlink
  (reasoner-sync tbox)
  (racer::atomic-role-ancestors2 role tbox))
...
(defun reasoner-atomic-role-descendants (role tbox)
  ;;; for OWLlink
  (reasoner-sync tbox)
  (racer::atomic-role-descendants2 role tbox))
...
(defun reasoner-kb-is-consistent-p (tbox abox)
  (reasoner-sync abox)
  (and (not
        (member 'top
...
(defun reasoner-inconsistent-concepts (tbox)
  (reasoner-sync tbox)
  (atomic-concept-synonyms 'cl-user::bottom tbox))
...
(defun reasoner-register-prefix (prefix namespace tbox)
  ...
  (when (is-default-prefix-p prefix)
    (racer::add-to-tbox-namespaces (find-tbox tbox)
...
(defun reasoner-syntax-message (message uri)
  (when (or ts::*really-warn-p*
    ...
    (format t message (- racer::*indent* 3) uri)))
