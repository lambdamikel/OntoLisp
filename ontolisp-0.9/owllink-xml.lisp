...
(declaim (special cl-user::*tcp-console-logging*))
...
(defun owllink2-eval-request (req ent)
  (racer:with-racer-critical-section
...
(defmacro process-owllink-request ()
  `(progn
     ...
     (#+:racer-server racer:with-racer-critical-section
      #-:racer-server progn
       
       (#+:racer-server racer:with-logging #+:racer-server (socket-stream)
        #-:racer-server progn
         ...
         (handler-case
           ...
           (error (c) 
             (error-printer 
              ...
              (let ((c 
                     #+:racer-server (racer:transform-to-html c)
                     #-:racer-server c))
                (if *error-indicator* 
                    (error-message "~A=~S"
                                   *error-indicator* c)
...
(defun owllink2-eval-request1 (parser-class socket-stream url)
  (let (#+:racer-server
	...
	#+:racer-server
        (logging cl-user::*tcp-console-logging*))
...
(defun owllink2-eval-request1 (parser-class req ent)
  (let (#+:racer-server
    ...
    (let ((url (net.aserve:request-uri req))
      ...
      #+:racer-server
      (when cl-user::*log-file*
        (with-open-file (log-stream cl-user::*log-file* :direction :output
			 :if-exists :append
...
(owlapi:owlapi-defun (|OWLAPI-readXMLOntologyFile|) (fn &rest args
  ...
  (let* ((*import-level* 0)
    ...
    (#+:racer-server racer:without-duplicate-warnings 
     #-:racer-server progn
